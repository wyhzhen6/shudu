...
public:
    ...
    void reverseBit(int i, int j, int digit)//使用位运算来表示某个数是否出现过
    {
...
void generateGame(int gameNumber, int gameLevel, vector<int> digCount, ofstream& outfile, SudokuMaker& maker)
{
    for (int i = 0; i < gameNumber; i++)
    {
        ...
        Board b = maker.generateBoard(cnt);
        vector<Board> bs;
        bs.push_back(b);
...
int main(int argc, char* argv[])
{
    ...
    while (current != params.end()) {
        ...
        case 'c':
            ...
            range.push_back(0);
            generateGame(atoi(value.c_str()), 0, range, outfile, maker);
            range.clear();
            break;
        ...
        case 'u': {
            ...
            outfile.open("game.txt", ios::out | ios::trunc);
            generateGame(gameNumber, gameLevel, range, outfile, maker);
            range.clear();
            break;
        }
        default:
